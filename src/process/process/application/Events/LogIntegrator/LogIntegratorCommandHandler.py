            ...
            if command.request.Exception is not None:
                if isinstance(command.request.Data, OperationBase):
                    ...
                    self.logger.exception(command.request.Exception, log_message,
                                          job_id=command.request.Data.Execution.Id)
                elif isinstance(command.request.Data, OperationIntegrationBase):
                    log_message = f'{command.request.Data.Id}-{command.request.Data.Order}-{command.request.Data.Name}-{command.request.Message}'
                    self.logger.exception(command.request.Exception, log_message,
                                          job_id=command.request.Data.Execution.OperationExecutionId)
                else:
                    log_message = f'{command.request.Message}'
                    self.logger.exception(command.request.Exception, log_message)
            else:
                if isinstance(command.request.Data, OperationBase):
                    log_message = f'{command.request.Data.Id}-{command.request.Data.Name}-{command.request.Message}'
                    self.logger.info(log_message, job_id=command.request.Data.Execution.Id)
                elif isinstance(command.request.Data, OperationIntegrationBase):
                    log_message = f'{command.request.Data.Id}-{command.request.Data.Order}-{command.request.Data.Name}-{command.request.Message}'
                    self.logger.info(log_message, job_id=command.request.Data.Execution.OperationExecutionId)
                else:
                    log_message = f'{command.request.Message}'
