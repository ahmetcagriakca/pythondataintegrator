...
from process.domain.common import Status, OperationEvent
from process.domain.operation import DataOperationJobExecution, DataOperationJobExecutionEvent, DataOperationJob
...
class ProcessOperationInitializer(OperationInitializer):
    ...
    def initialize(self, operation: OperationBase):
        if operation.Execution is None or (operation.Execution.Id is None and operation.Execution.ApSchedulerJobId is None):
        ...
        if operation.Execution is not None and operation.Execution.Id is None:
            data_operation_job = self.get_data_operation_job_by_operation_and_job_id(
                data_operation_id=operation.Id,
                ap_scheduler_job_id=operation.Execution.ApSchedulerJobId)
            if data_operation_job is None:
                error = f'{operation.Id}-{operation.Execution.ApSchedulerJobId} Data operation job not found'
