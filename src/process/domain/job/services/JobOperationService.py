...
class JobOperationService(IScoped):
    ...
    def add_job_with_date(self, job_function, run_date, args=None, kwargs=None) -> ApSchedulerJob:
        if self.application_config.process_node:
            conn = rpyc.connect('localhost', 7300)
            job = conn.root.add_job_with_date('server:domain.job.services.JobOperationService:JobOperationService.job_start_data_operation', run_date.strftime("%Y-%m-%dT%H:%M:%S.%fZ"), args=['Hello, World'], seconds=2)
        else:
