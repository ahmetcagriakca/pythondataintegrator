...
def insert_connection_server():
    from pdi.domain.connection import Connection, ConnectionServer
    bind = op.get_bind()
    from sqlalchemy import orm
    session = orm.Session(bind=bind)
    query = bind.execute(
        'select dic."ConnectionId", dic."Host",dic."Port" from "Connection"."ConnectionDatabase"  as dic')
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('ConnectionFile', sa.Column('Port', sa.INTEGER(), autoincrement=False, nullable=True),
                  schema='Connection')
    op.add_column('ConnectionFile', sa.Column('Host', sa.VARCHAR(length=100), autoincrement=False, nullable=True),
                  schema='Connection')
    op.add_column('ConnectionDatabase', sa.Column('Port', sa.INTEGER(), autoincrement=False, nullable=True),
                  schema='Connection')
    op.add_column('ConnectionDatabase', sa.Column('Host', sa.VARCHAR(length=100), autoincrement=False, nullable=True),
                  schema='Connection')
