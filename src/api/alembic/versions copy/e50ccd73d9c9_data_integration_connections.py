...
def insert_data_integration_connection_to_database():
    from pdi.domain.integration import DataIntegrationConnection, DataIntegrationConnectionDatabase
    bind = op.get_bind()
    from sqlalchemy import orm
    session = orm.Session(bind=bind)
    query = bind.execute(
        'select dic."Id", dic."Schema",dic."TableName",dic."Query" from "Integration"."DataIntegrationConnection"  as dic')
    results = query.fetchall()
    for data_integration_connection_data in results:
        data_integration_connection = session.query(DataIntegrationConnection).filter_by(
            Id=data_integration_connection_data[0]).first()
        data_integration_connection_database = DataIntegrationConnectionDatabase(
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('DataIntegrationConnection', sa.Column('Query', sa.TEXT(), autoincrement=False, nullable=True),
                  schema='Integration')
    op.add_column('DataIntegrationConnection',
                  sa.Column('Schema', sa.VARCHAR(length=100), autoincrement=False, nullable=True), schema='Integration')
    op.add_column('DataIntegrationConnection',
                  sa.Column('TableName', sa.VARCHAR(length=100), autoincrement=False, nullable=True),
                  schema='Integration')
