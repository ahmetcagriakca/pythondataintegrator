...
def insert_connection_database_to_secrets():
    from pdi.domain.connection import Connection, ConnectionSecret
    from pdi.domain.secret import Secret, SecretSource, AuthenticationType, SecretType, SecretSourceBasicAuthentication
    from pdi.domain.enums import SecretTypes, AuthenticationTypes
    bind = op.get_bind()
    from sqlalchemy import orm
    session = orm.Session(bind=bind)
    connection_database_query = bind.execute(
        'select c."Name",cd."User",cd."Password" from "Connection"."ConnectionDatabase" as cd join "Connection"."Connection" as c on c."Id" = cd."ConnectionId"')
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('ConnectionDatabase',
                  sa.Column('Password', sa.VARCHAR(length=100), autoincrement=False, nullable=True),
                  schema='Connection')
    op.add_column('ConnectionDatabase', sa.Column('User', sa.VARCHAR(length=100), autoincrement=False, nullable=True),
                  schema='Connection')
