...
from src.application.connection.CreateConnectionBigData.CreateConnectionBigDataCommand import \
...
class CreateConnectionBigDataCommandHandler(ICommandHandler[CreateConnectionBigDataCommand]):
    @inject
    def __init__(self,
    ...
    def handle(self, command: CreateConnectionBigDataCommand):
        check_existing = self.connection_service.check_connection_name(command.request.Name)
        connection = self.connection_service.create_connection_big_data(command.request)
    ...
    def notify(self, id: int, name: str, check_existing: bool):
        data_list = []
        data = NotificationAdditionalData(Key="Type", Value="ConnectionBigData")
