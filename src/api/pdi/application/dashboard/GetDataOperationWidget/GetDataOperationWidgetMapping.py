...
class GetDataOperationWidgetMapping:
    widget_data: str = """
    {
...
"""
    ...
    @classmethod
    def to_dto(cls, query: Query) -> GetDataOperationWidgetDto:
        dto = GetDataOperationWidgetDto()

        widget_data = yaml.load(cls.widget_data, yaml.FullLoader)
        today_count = query.filter(DataOperation.IsDeleted == 0).first().Count
        widget_data['data']['value'] = today_count
        yesterday_count = query \
            .filter(
            or_(
                and_(DataOperation.IsDeleted == 0, DataOperation.CreationDate < date.today()),
                and_(DataOperation.IsDeleted == 1, DataOperation.LastUpdatedDate > date.today())
            )
        ) \
            .first().Count
        ...
        for date_range in range(7):
            ...
            result = query \
                .filter(
                or_(
                    and_(DataOperation.IsDeleted == 0, DataOperation.CreationDate < day),
                    and_(DataOperation.IsDeleted == 1, DataOperation.LastUpdatedDate > day)
                )
            ) \
                .first().Count
