...
import { getConnectionSql, postConnectionSql, checkSqlConnection, clearSqlConnection, checkSqlTableRowCount, deleteSqlConnection } from './store/connectionSqlSlice';
...
function ConnectionSqlContent() {
	...
	const save = event => {
		dispatch(postConnectionSql(values));
	};

	const checkConnectionAction = event => {
		routeParams.ConnectionName = values.name
		dispatch(checkSqlConnection(routeParams));
	};

	const deleteAction = event => {
		dispatch(deleteSqlConnection({ id: parseInt(routeParams.id) }));
	};
	...
	const checkCountAction = event => {
		...
		dispatch(checkSqlTableRowCount(routeParams));
	};
	return (
