...
export const getConnectorTypeName = createAsyncThunk('connectionBigDataApp/connectorTypeName/getConnectorTypeName', async (params, extra) => {
	try {
...
export const { selectAll: selectConnectorTypeName, selectById: selectConnectorTypeNameById } = connectorTypeNameAdapter.getSelectors(
	state => state.connectionBigDataApp.connectorTypeName
);

const connectorTypeName = createSlice({
	name: 'connectionBigDataApp/connectorTypeName',
