...
class SqlLogger(IScoped):
    ...
    def __init__(self,
        ...
        self.python_data_integration_repository: Repository[PythonDataIntegrationLog] = Repository[
    ...
    def log_to_db(type_of_log, log_string, job_id=None):
        ...
        python_data_integration_log_repository: Repository[PythonDataIntegrationLog] = Repository[
            PythonDataIntegrationLog](database_session_manager)
        log_datetime = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
        comment = f'Python Data Integrator {api_config.environment}'
        try:
            log = PythonDataIntegrationLog(TypeId=type_of_log, Content=log_string[0:4000], LogDatetime=log_datetime,
                                           JobId=job_id, Comments=comment)
            python_data_integration_log_repository.insert(log)
